...
```{r}
oceanus_map = st_read("data/MC2/Oceanus Information/Oceanus Geography.geojson") %>%
...
```{r}
write_rds(oceanus_map, "data/rds/oceanus_map.rds")
```
...
First, we will extract the relevant nodes, namely the vessels and locations from `mc2_nodes`. As we are only concerned about the fishing vessels and cargo vessels, we will only extract values matching "Vessel" in entity2 column and values matching "FishingVessel" & "CargoVessel" in entity3 column. For locations, we will match values of "Location" in entity2 column, and match values of "City", "Point" and "Region" in entity3 column.

```{r}
unique(mc2_nodes$entity3)
```
...
vessel_mvmt_nodes <- mc2_nodes %>% 
  ...
  filter(entity3 %in% c("FishingVessel","CargoVessel","City","Point","Region"))
```
