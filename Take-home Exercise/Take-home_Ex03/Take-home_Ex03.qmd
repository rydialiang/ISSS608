...
transponder_ping_edge %>% 
  ...
  geom_hline(average, 
             ...
             color="black",
             linetype = 6) +
  geom_point(x = 5, y = 14260,
             size = 4, color = "red") +
  geom_text(aes(x = 5, y = 14260, label = "SouthSeafood caught"),
            color = "red",
            hjust = -0.2,
            size = 3) +
  geom_text(aes(x = 4.5, y = 12900, label = "Monthly Average"),
            color = "black",
            size = 3)
...
ggplot(monthly_transponder_ping , aes(x = month, 
  ...
  theme_minimal() +
  labs(title = "Changes in Visit Frequency by Month",
       x = "Month",
       y = "No. of Visits",
       color = "Source") +
...
```
From the plot above, we can see that after SouthSeafood was caught in mid-May, there is a drop in activities for all the fishing grounds and preserves immediately after mid-May, except for the increase in Don Limpet Preserve. 
...
don_monthly_transponder_ping/nemo_monthly_transponder_ping/ghoti_monthly_transponder_ping
```
